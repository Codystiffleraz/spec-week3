...
class Melon:
    ...
    def __repr__(self):  # convenience method to show information about melon in console
        return (
            f"<Melon: {self.melon_id}, {self.common_name}>"
        )

    def price_str(self):  # returns a price formatted as string $x.xx
...
def get_by_id(melon_id):  # returns a melon, given its ID
    return melon_dict[melon_id]


def get_all():  # returns a list of melons
